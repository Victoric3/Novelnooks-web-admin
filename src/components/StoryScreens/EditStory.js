...
const EnhancedBookEditor = () => {
  ...
  useEffect(() => {
    try {
      const storedBooks = JSON.parse(localStorage.getItem('authorBooks') || '[]');
      ...
      if (bookToEdit) {
        ...
        const initialChapters = editMode === 'full' 
          ? [createChapter(0, 'Write your new preface here...')]
          : Array.from({ length: bookToEdit.contentCount }, (_, index) => ({
              index,
              content: localStorage.getItem(`EditedContent-${index}`) || "",
              isEdited: false,
              originalIndex: index,
              placeholder: index === 0 ? "Write your preface here..." : 
  ...
  const ChapterEditor = React.memo(({ chapter, onChange, onDelete, isAddable = false }) => {
...
const handleChapterChange = useCallback((index, content) => {
  setChapters(prev => 
    ...
    localStorage.setItem(`EditedContent-${index}`, content);
  }, [])
  
  const handleDeleteChapter = useCallback((indexToDelete) => {
    setChapters(prev => 
      prev.filter(chapter => chapter.index !== indexToDelete)
        .map((chapter, index) => ({
          ...chapter,
          index: index
        }))
  ...
  const renderMetadataSection = () => {
    return (
      <div className="metadata-section">
        ...
        >
          <h2>Book Metadata</h2>
          {collapsedSections.metadata ? <IoMdArrowDropdown /> : <IoMdArrowDropup />}
        </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (partial) {
        const editedChapters = chapters
          .filter(chapter => chapter.isEdited)
      ...
      console.log('partial: ', partial, 'content', content, 'summary: ', summary, 'tags: ', tags, 'title: ', title)
          ...
          {['chapter', 'full', 'add'].map(mode => (
            <button 
              ...
              onClick={() => {
                setEditMode(mode)
                if (mode === 'full') {
                  setChapters([createChapter(0, 'Write your new preface here...')]);
                }
              }}
            >
              {mode === 'full' ? 'Edit Entire Book' : 
               mode === 'chapter' ? 'Edit Specific Chapters' : 
               'Add New Chapters'}
            </button>
          ))}
        </div>
